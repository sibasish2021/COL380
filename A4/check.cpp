...
vector<Block> InputData1(string fileName,int &n,int &m,int &k)
{
    vector<Block> v;
...
vector<Block> InputData2(string fileName,int &n,int &m,int &k)
{
    vector<Block> v;
    ifstream f1(fileName);
    f1.read((char *)&n,sizeof(int32_t));
...
void compareBinary(string file1,string file2)
{
    ...
    b1=InputData1(file1,n1,m1,k1);
    b2=InputData2(file2,n2,m2,k2);
    ...
    cout<<"n,m,k are matching"<<endl;
    if(b1.size()!=b2.size())
    {
        cout<<"Block size mismatch"<<b1.size()<<" "<<b2.size()<<endl;
        return;
    }
    cout<<"Number of blocks are matching"<<endl;
    for(int i=0;i<b1.size();i++)
    {
        ...
        for(int j=0;j<m1;j++)
        {
            for(int k=0;k<m1;k++)
            {
                ...
                {
                    cout<<"Elements do not match at block"<<b1[i].posx<<" "<<b1[i].posy <<endl;
                    cout<<"The index is "<<j<<" "<<k<<endl;
